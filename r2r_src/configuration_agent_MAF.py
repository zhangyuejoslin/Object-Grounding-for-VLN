...
class Seq2SeqAgent(BaseAgent):
    ...
    def _candidate_variable1(self, obs):
        ...
        return torch.from_numpy(candidate_feat).cuda(), candidate_leng, torch.from_numpy(candidate_obj_img_feat).cuda(), torch.from_numpy(object_mask).cuda(), torch.from_numpy(object_index).cuda(),\
             torch.from_numpy(object_relation).cuda(), object_text
    ...
    def get_input_feat(self, obs):
        ...
        f_t, pano_obj_feat, pano_obj_mask, pano_obj_text = self._feature_variable(obs)      # Image features from obs
        candidate_feat, candidate_leng, candidate_obj_text_feat, object_mask, object_index, object_relation, object_text= self._candidate_variable1(obs)
        ...
        return input_a_t, f_t, candidate_feat, candidate_obj_text_feat, object_mask, candidate_leng, pano_obj_feat, object_index, object_relation, \
            object_text, pano_obj_mask, pano_obj_text
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True, speaker=None):
       ...
       # ctx, h_t, c_t = self.encoder(sentence_list, seq_lengths)
        ...
        for t in range(self.episode_len):
            ...
            input_a_t, f_t, candidate_feat, candidate_obj_img_feat, candi_object_mask, candidate_leng, pano_obj_img_feat, object_index, object_relation, candi_object_text, pano_obj_mask, pano_object_text = self.get_input_feat(perm_obs)
            ...
            h_t, c_t, logit, h1, ctx_attn = self.decoder(input_a_t, f_t, candidate_feat,
                                               ...
                                               candidate_obj_feat = candi_maf_object_repr , landmark_mask=landmark_mask, landmark_relation=landmark_relation, landmark_relation_mask=landmark_relation_mask, candidate_relation=object_relation,
                                               pano_obj_feat=pano_maf_object_repr, pano_landmark = pano_maf_landmark_repr, pano_obj_mask=pano_obj_mask, object_index =object_index,
