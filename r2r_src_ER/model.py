...
class ConfigurationDecoder(nn.Module):
    ...
    def forward(self, action, feature, cand_feat,
        ...
        pano_sim_index = torch.max(self.cos(pano_obj_feat1, landmark_object_feature1.unsqueeze(1)).view(batch_size, pano_img_num, pano_obj_num, -1),dim=2)[1]
        ...
        top_N_sim_obj_index =torch.sort((s_0.unsqueeze(-1).repeat(1,1,landmark_num)*landmark_mask).view(batch_size,-1),descending=True)[1][:,:top_n]
        ...
        landmark_object_feature2 = landmark_object_feature.view(batch_size,-1,obj_feat_dim).unsqueeze(dim=1).repeat(1,object_num,1,1).view(batch_size,-1,obj_feat_dim)
        ...
        candi_sim_index = torch.max(self.cos(candidate_obj_text_feat1, landmark_object_feature2.unsqueeze(1)).view(batch_size, image_num, object_num, -1),dim=2)[1]
        candi_top_N_sim_obj_index =torch.sort((ctx_attn.unsqueeze(-1).repeat(1,1,landmark_num)*landmark_mask).view(batch_size,-1),descending=True)[1][:,:top_n]
...
class ConfigurationRelationDecoder(nn.Module):
    ...
    def forward(self, action, feature, cand_feat,
                ...
                candidate_obj_img_feat = None, candidate_obj_text_feat = None, landmark_mask=None, landmark_relation=None, candidate_relation=None,
        ...
        candi_sim_rel = torch.cat((candidate_relation.unsqueeze(2).expand(batch_size,image_num,top_n,6),land_obj_relation.unsqueeze(-1)),dim=-1)
