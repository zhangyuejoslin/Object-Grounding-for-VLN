...
import encoder
from MAF.R2R_task.train_model import MAF_repr
from MAF.R2R_task.utils.utils import load_vocabulary, init_net
from MAF.R2R_task.model import MATnet
...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, env, results_path, tok, episode_len=20):
        ...
        self.decoder = model.ConfigurationMAFDecoder(args.aemb, args.rnn_dim, args.dropout, feature_size=self.feature_size + args.angle_feat_size).cuda()
        ...
        self.net.load_state_dict(torch.load("/VL/space/zhan1624/MAF/Multimodal-Alignment-Framework/saved_model/not_pre_train/best-model-parameters.pt"))
        self.net.eval()
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True, speaker=None):
       ...
       # ctx, h_t, c_t = self.encoder(sentence_list, seq_lengths)
        tmp_ctx, h_t, c_t, tmp_ctx_mask, split_index = self.encoder(sentence, seq_len=80)
        ...
        for t in range(self.episode_len):
            ...
            input_a_t, f_t, candi_feat, candi_obj_img_feat, candi_object_mask, candidate_leng, pano_obj_img_feat, object_index, \
            object_relation, candi_object_text, pano_obj_mask, pano_object_text, pano_obj_text_feat, candi_obj_text_feat = self.get_input_feat(perm_obs)
            candi_obj_img_feat = candi_obj_img_feat.cuda()
            candi_object_mask = candi_object_mask.cuda()
            candi_obj_text_feat = candi_obj_text_feat.cuda()
            ...
            '''
            new_candi_leng = list(map(lambda x:x-1,candidate_leng))
            new_candi_mask = utils.length2mask(new_candi_leng, size=max(candidate_leng))
            pano_landmark_text = []
            ...
            new_object_text = list(itertools.chain(*candi_object_text))
            
            candi_maf_landmark_repr, candi_maf_object_repr = MAF_repr(self.wordEmbedding, self.net, new_object_text, 
                                                                      candi_landmark_text, candi_obj_img_feat.view(batch_size*candi_image_num, 36, 2048), 
                                                                      max_config_num*max_landmark_num)
            ...
            h_t, c_t, logit, h1, ctx_attn = self.decoder(input_a_t, f_t, candi_feat,h_t, h1, c_t,
                                                         ctx, t, ctx_attn, ctx_mask, candi_object_mask=candi_object_mask,
                                                         candi_landmark=candi_maf_landmark_repr, candi_img_feat=candi_maf_object_repr, 
                                                         candi_text_feat=candi_obj_text_feat, landmark_mask=landmark_mask, 
