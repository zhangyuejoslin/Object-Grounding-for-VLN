...
from configuration_agent_MAF import Seq2SeqAgent
...
def train_val():
    ...
    if args.using_obj:
        ...
        #pano_caffe = None
        pano_caffee_text = np.load(args.obj_text_feat_path, allow_pickle=True).item()
    ...
    train_env = R2RBatch(feat_dict, pano_caffe,  pano_caffee_text, batch_size=args.batchSize, splits=['train'], tokenizer=tok)
    ...
    val_envs = OrderedDict(
        ((split,
          (R2RBatch(feat_dict, pano_caffe,  pano_caffee_text, batch_size=args.batchSize, splits=[split], tokenizer=tok),
